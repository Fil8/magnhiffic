...
def sample_table(rows):
    """rows: list of dicts with name/ra/dec/dl_mpc/gal_type/agn_type/environment/ned_url"""
    ...
    for r in rows:
        ...
        if r.get("ned_url"):
            nm = ('<a href="%s" target="_blank" class="u-active-none '
                  ...
                  'u-none u-text-hover-palette-1-base u-text-white">%s</a>'
                  % (r["ned_url"], r["name"]))
        cells = [nm, r["ra"], r["dec"], r["dl_mpc"] or "\u2013",
                 r["gal_type"], r["agn_type"], r["environment"]]
        tds = "".join('\n                <td class="u-border-1 u-border-grey-30 '
                      'u-table-cell">%s</td>' % c for c in cells)
        trs += ('\n              <tr style="height: 40px;">%s\n              </tr>'
                % tds)
