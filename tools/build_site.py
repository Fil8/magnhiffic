...
def build_public_gallery():
    ...
    # Same markup as the Home strip: the title is a plain line above each
    # picture rather than the framework gallery's heading painted over it.
    # (The classes are still named u-home-gallery-*; the CSS below is this
    # page's own, laying them out as a grid instead of a single row.)
    gal = home_gallery_strip(gallery_items())
...
/* A grid rather than the Home strip's single flex row, so a third image
   wraps onto a second row instead of squeezing the first. */
.u-section-2 .u-home-gallery {
  display: grid;
